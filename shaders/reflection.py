...
if not hasattr(g, 'grid_reflection'):
	g.grid_reflection = texture.Texture(own, 0, 0)
	g.grid_reflection.source = texture.ImageRender(scene,watercamera)
	g.grid_reflection.source.capsize = [reflsize,reflsize]
	g.grid_reflection.source.background = [int(bgR*255),int(bgG*255),int(bgB*255),int(bgA*255)]

g.grid_reflection.refresh(True)
